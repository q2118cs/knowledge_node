...
| retries          | 重试次数。注意：一般设置重试会导致消息顺序不一致，如果还想保持消息的有序性，需要设置MAX_IN_FLIGHT_REQUESTS_PRE_CONNECTION=1 |
| batch.size       | 默认批次大小。发送到Broker的请求包含多个批次，如果该值设置的比较小会限制吞吐量，如果设置的很大会浪费内存。 |
| linger.ms        | 消息累积成批次的时间，默认值为0。该参数和batch.size配合使用，如果消息达到了 batch.size消息批次会立即发送，如果积累的消息达不到 batch.size，等待该参数设置的时间后也会发送消息批次。 |

## 消费者

多个从一个topic消费的消费者组成一个消费组，消费组要设置相同的group_id，每个分区只能有消费组中的一个消费者来消费。

消费者宕机的时候，会退出消费组，触发再平衡，重新给消费组中的消费者分配分区，通过心跳检查机制来确认。

consumer采用pull模式从broker拉取数据。主要因为kafka应用在海量数据场景，consumer可自主处理消息。
...
1. 消费组内成员发生变化；
2. 主题的分区数发生变化；
3. 订阅的主题发生变化，比如使用正则的方式订阅主题，这时候新建了主题就会触发重平衡。

重平衡过程中消费者无法消费消息，对kafka的TPS影响很大，所以要避免重平衡的发生，可以通过三个参数来尽力避免重平衡的发生：

- session.timout.ms控制心跳超时时间，一般设置为6s
- heartbeat.interval.ms控制心跳发送频率，一般设置为2s
- max.poll.interval.ms控制poll的间隔，一般设置为消费者处理消息最长耗时再加1分钟
